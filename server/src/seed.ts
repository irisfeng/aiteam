 ...
 */
export const BUILTIN_SKILL_PACK_VERSION = 8;
...
export const BUILTIN_SKILLS: BuiltinSkill[] = [
...

  {
    ...
    resources_json: JSON.stringify(["tpl:html-deck-horizontal"]),
    body: "结论先行：**完整对外方案优先用 start_project 立项、多角色协作**，不要一个人从头做到尾（一人包办既慢又不过验收）。1) 标准任务 DAG（带 depends_on）：① 调研员/产品经理——需求澄清 + 市场/竞品调研（关键数字带源、注明检索日期）；② 工程师——技术选型与架构（依赖①，给对比表 + 推荐理由）；③ 产品经理/解决方案助手——整合为完整方案 report/PRD（依赖①②）；④ PPT 助手——做对外演示 slides（依赖③）；⑤ 校对审核/代码评审——逐条验收（依赖④）；⑥ 你(Lead)——汇总。2) 叙事主线（金字塔）：痛点/现状 → 方案概览(一句话价值) → 总体架构 → 关键能力 → 选型对比 → 量化价值/ROI → 实施路线 → 风险保障 → 标杆案例 → CTA。3) slides 质量：关键指标用 `值 :: 标签` 数字卡、对比/选型用 Markdown 表格、分段用『只含 # 标题』章节幕页、每页配 `<!-- note: … -->` 备注、正文纯 Markdown 禁原始 HTML；用户在文档面板一键 ⬇ .pptx。4) 配图：封面/章节/概念页可用 generate_image 出 1-2 张点睛图（Seedream 已就位）；**架构图/流程图不要用文生图（会糊中文标签）**，用结构化表格 / 清晰文字层级表达。5) 数字接地：市场/降本/ROI 等无可靠来源就标『示意值，待核实』，绝不编造；先 read_document 查工作区有无现成材料。6) 只改一页/只写一段这类单环节小活才不立项、直接做。配合『金字塔写作法』『深度调研法』『演示设计与防溢出法』。",
  },
