...
- **Mission 服务鉴权**：Coworker 服务端用独立的
  `AITEAM_SERVICE_JWT_SECRET` 签发 60 秒 HS256 JWT；轮换时 AiTeam 使用
  `AITEAM_SERVICE_JWT_KEYS` 同时接受 current/previous `kid`。AiTeam 校验固定
  issuer/audience/subject、组织和 scope。服务密钥不得复用 `AUTH_SECRET`。
...
> 不同的 URL、服务密钥和数据目录。
> 轮换顺序必须是：AiTeam 先加入 current+previous keyring，Coworker 再切换
> `AITEAM_SERVICE_JWT_KEY_ID` 与新 secret，至少等待 60 秒令牌窗口后，AiTeam
> 才删除 previous key。
