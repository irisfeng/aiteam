...
**质量与产出**
- **干净上下文验收闭环**：换一个同事按 rubric 逐条核验交付物（多份交付一并核验，固定校验者判准不随人选漂移），未过退回返工；无结构化裁决时 fail-closed 退回（不放水）
- **质量账本（内建 evals）**：每次验收裁决落表可回放（第几轮 / pass·revise / 机器·自检·人工 / 理由全文）；`/quality` 汇总各同事的一次通过率（首轮即通过的任务占比）、近期返工原因、验收覆盖率；达返工上限时未解决差距结构化带给人工复核
...
- **成本护栏与可预测性**：任务级累计成本（工作+返工+验收全归因、跨返工不清零）、开工前按历史同档任务给估价、任务/全局预算触线自动暂停待批（批准即追加预算续跑）；每用户每日 token 预算硬切断、MCP/配图按次封顶、MCP 调用超时自愈
